...
class Video2Images:
    ...
    def __init__(self,
        ...
        # Folder time stamp
        folder_name = self.out_folder_name
    ...
    def __start(self):
        ...
        cycle = int(FPS/self.capture_rate)

        total_frames = int(round(duration)*meta_data['fps'])
        image = 0
        running_video_timer = 0
        time_between_frames = cycle/FPS

        for frame in range(1, total_frames, cycle):
            image += 1
            ...
            running_video_timer += time_between_frames

        print(f"\033[1;32;40m Done. Total frames captured: {image - 1} \033[00m")
        print("Time taken for process:" + str((time.time() - start_time)))
        #iio.imiter()
        '''
